...
var GTE;
(function (GTE) {
    var SCell = (function (_super) {
        __extends(SCell, _super);
        function SCell(game, x, y, group) {
            var _this = _super.call(this, game, x, y, game.cache.getBitmapData("cell")) || this;
            group.add(_this);
            _this.game = game;
            _this.anchor.set(0, 0);
            _this.p1Text = _this.game.add.text(_this.position.x + _this.width * 0.1, _this.position.y + _this.width * 0.9, Math.round(Math.random() * 20).toString(), null);
            ...
            _this.p2Text = _this.game.add.text(_this.position.x + _this.width * 0.9, _this.position.y + _this.width * 0.1, Math.round(Math.random() * 20).toString(), null);
